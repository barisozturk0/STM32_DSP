...
static float32_t sinc(float32_t x);
extern float32_t inputSignal_f32_1kHz_10kHz[KHZ_1_10_SIG_LEN];
static float32_t output[KHZ_1_10_SIG_LEN+FILTER_LENGTH];
...
int main(void){
    ...
    arm_conv_f32(inputSignal_f32_1kHz_10kHz, KHZ_1_10_SIG_LEN, coeff_out, FILTER_LENGTH, output);
    //convolution(inputSignal_f32_1kHz_10kHz, coeff_out, output);


  for(int i=0;i<KHZ_1_10_SIG_LEN+FILTER_LENGTH;i++){
...
void impulse_response_f(float32_t Cuttoff_Freq,float32_t Sampling_Freq,uint32_t Filter_length ){
	...
	for(int n=0;n<Filter_length;n++){
      ...
      if(n==M){
    	  sinc_out[n]=1.0-((2*Cuttoff_Freq)/Sampling_Freq);
    	  ...
    	  sinc_out[n]=-2.0*(Cuttoff_Freq/Sampling_Freq)*sinc(x);
...
void  filter_coeff(uint32_t Filter_length){
 ...
 static void convolution(float32_t*input_signal,float32_t*impulse_response,float32_t*output){
	...
	uint32_t length=KHZ_1_10_SIG_LEN+FILTER_LENGTH;
